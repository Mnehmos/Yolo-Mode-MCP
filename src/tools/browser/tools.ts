...
export async function handleGetPageContent(args: { format?: 'html' | 'text' | 'markdown' }) {
    try {
        ...
        const content = await manager.getProvider().getContent(format);
        await logAudit('get_page_content', args, `retrieved ${content.length} chars`);
        return {
...
export async function handleEvalJs(args: { script: string }) {
    try {
        ...
        await logAudit('evaluate_js', { scriptLength: args.script.length }, 'success');
        return {
            content: [{
                type: 'text',
...
export async function handleGetConsoleLogs() {
    try {
        const logs = await manager.getProvider().getConsoleLogs();
        await logAudit('get_console_logs', {}, `retrieved ${logs.length} logs`);
        return {
            content: [{
                type: 'text',
