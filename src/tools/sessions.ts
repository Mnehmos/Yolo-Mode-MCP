    ...
    try {
        ...
        if (args.lines !== undefined) {
            if (args.lines < 0) {
                ...
                output = session.output.slice(args.lines);
            } else {
                // Positive: first N lines
                output = session.output.slice(0, args.lines);
            }
        } else {
        ...
        await logAudit('read_process_output', { sessionId: args.sessionId, linesRequested: args.lines }, { linesReturned: output.length });
